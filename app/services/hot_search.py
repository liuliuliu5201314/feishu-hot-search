...
class HotSearchService:
    ...
    def write_hot_data(self, bilibili_items, weibo_items, zhihu_items):
        ...
        fields = {"抓取时间": timestamp_ms}
        
        for i, item in enumerate(bilibili_items[:5], 1):
            fields[f"B站热搜{i}"] = item.get("keyword", "")
        
        for i, item in enumerate(weibo_items[:5], 1):
            fields[f"微博热搜{i}"] = item.get("keyword", "")
        
        for i, item in enumerate(zhihu_items[:5], 1):
            fields[f"知乎热搜{i}"] = item.get("keyword", "")
